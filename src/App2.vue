...
async function applyConfig() {
  ...
  if (!validateIpAddress(ipConfig.gateway)) {
    statusMsg.value = "网关地址格式不正确，请使用xxx.xxx.xxx.xxx格式";
    return;
  }
  
  if (!validateIpAddress(ipConfig.dns1)) {
    statusMsg.value = "DNS1地址格式不正确，请使用xxx.xxx.xxx.xxx格式";
    return;
  }
  
  if (!validateIpAddress(ipConfig.dns2)) {
    statusMsg.value = "DNS2地址格式不正确，请使用xxx.xxx.xxx.xxx格式";
    return;
  }
...
async function getCurrentConfig() {
  ...
  try {
    const config = await invoke<IpConfig>("get_current_config", { 
      adapter_name: selectedAdapter.value 
    });
